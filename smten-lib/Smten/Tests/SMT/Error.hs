...
smttests = do
   ...
   symtesteq "SMT.Error.IntegerNoFair" (Just ()) $ do
      p <- free_Bool
      assert p
      assert ((if p then 1 else error "SMT.Error.IntegerNoFair error hit") == (1 :: Integer))

   symtesteq "SMT.Error.IntegerFair1" (Just ()) $ do
      p <- free_Bool
      assert ((if p then 1 else error "SMT.Error.IntegerFair1 error hit") == (1 :: Integer))

   symtesteq "SMT.Error.IntegerFair2" (Just ()) $ do
      p <- free_Bool
      assert ((if p then error "SMT.Error.IntegerFair1 error hit" else 1) == (1 :: Integer))
   ...
   symtesteq "SMT.Error.IntNoFair" (Just ()) $ do
      p <- free_Bool
      assert p
      assert ((if p then 1 else error "SMT.Error.IntNoFair error hit") == (1 :: Int))

   symtesteq "SMT.Error.IntFair1" (Just ()) $ do
      p <- free_Bool
      assert ((if p then 1 else error "SMT.Error.IntFair1 error hit") == (1 :: Int))

   symtesteq "SMT.Error.IntFair2" (Just ()) $ do
      p <- free_Bool
      assert ((if p then error "SMT.Error.IntFair1 error hit" else 1) == (1 :: Int))

   symtesteq "SMT.Error.CharNoFair" (Just ()) $ do
      p <- free_Bool
      assert p
      assert ((if p then 'a' else error "SMT.Error.CharNoFair error hit") == 'a')

   symtesteq "SMT.Error.CharFair1" (Just ()) $ do
      p <- free_Bool
      assert ((if p then 'a' else error "SMT.Error.CharFair1 error hit") == 'a')

   symtesteq "SMT.Error.CharFair2" (Just ()) $ do
      p <- free_Bool
      assert ((if p then error "SMT.Error.CharFair1 error hit" else 'a') == 'a')
